...
void drawSun() 
{
	...
	glm::vec3 sunPos(C[0], C[1], C[2]);
	glm::vec3 playerPos(game.getPlayerLoc()[0], 1, game.getPlayerLoc()[1]);
	glDisable(GL_DEPTH_TEST);
	glDisable(GL_LIGHTING);
	for(int i=0; i<5; ++i)
	{
		float alpha = (float)i/5;
		glm::vec3 pos = sunPos*(1-alpha) + playerPos*alpha;
		glPushMatrix();
